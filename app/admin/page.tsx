...
export default function Admin() {
  ...
  async function load(u: any) {
    ...
    const { data: c, error: ce } = await sb.rpc('get_my_admin_context');
    const nextContext = !ce && c?.[0] ? c[0] as AdminContext : null;
    setContext(nextContext);
    if (!nextContext?.profile_id) { setLoading(false); return; }
    const { data: p } = await sb.from('profiles').select('*').eq('id', nextContext.profile_id).maybeSingle();
    setProfile(p);
    if (p) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = sb.auth.onAuthStateChange((_event, session) => load(session?.user));
  ...
  useEffect(() => {
    ...
    const reset = () => {
      ...
      timer = setTimeout(async () => { await sb.auth.signOut(); }, 60 * 60 * 1000);
    };
  ...
  async function auth() {
    ...
    setMsg(error?.message || text('ready', lang));
  }
  ...
  async function saveProfile() {
    if (!profile || !context?.permissions.manage_profile) return;
  ...
  async function uploadLogo(file: File) {
    if (!profile || !context?.permissions.manage_assets) return;
  ...
  async function saveLink(l: LinkItem, i: number) {
    if (!profile || !context?.permissions.manage_links) return;
  ...
  function updateProfileTranslation(field: 'name'|'handle'|'bio'|'footer', value: string) {
    if (!profile || !context?.permissions.manage_translations) return;
  ...
  function updateLinkTranslation(i: number, field: 'title'|'subtitle', value: string) {
    if (!context?.permissions.manage_translations) return;
  ...
  const qrUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/${profile?.slug || ''}`;

  if (loading) return <main className="login"><div className="login-card">{text('loading', lang)}</div></main>;
  if (!user) return <main className="login"><div className="login-card"><h1>{text('admin', lang)}</h1><p className="muted">{text('adminDesc', lang)}</p><div className="field"><label>{text('email', lang)}</label><input value={email} onChange={e => setEmail(e.target.value)} type="email" autoComplete="email" /></div><div className="field"><label>{text('password', lang)}</label><input value={password} onChange={e => setPassword(e.target.value)} type="password" autoComplete="current-password" onKeyDown={e=>{if(e.key==='Enter')auth()}} /></div><button className="btn btn-primary" style={{width:'100%'}} onClick={auth}>{text('login',lang)}</button><button className="btn btn-secondary" style={{width:'100%',marginTop:9}} onClick={google}>Google</button>{msg&&<p>{msg}</p>}<div className="language-tabs" style={{marginTop:16,justifyContent:'center'}}>{langs.map(x=><button type="button" key={x.id} className={`language-circle ${lang===x.id?'active':''}`} title={x.name} aria-label={`${text('editLanguage',lang)} ${x.name}`} onClick={()=>setLang(x.id)}>{x.label}</button>)}</div></div></main>;
  if (!context || !profile) return <main className="login"><div className="login-card"><h1>{text('profileNotFound', lang)}</h1><p className="muted">{text('profileNotFoundDesc', lang)}</p><button className="btn btn-secondary" onClick={()=>sb.auth.signOut()}>{text('logout',lang)}</button></div></main>;
  ...
  return <main className="admin-page"><div className="admin-wrap">
    <header className="admin-head"><div><span className="eyebrow">IGLESIA LINKTREE</span><h1>{text('admin',lang)}</h1><p className="muted">{context.role}{context.is_primary?' · primary':''}</p></div><button className="btn btn-secondary" onClick={()=>sb.auth.signOut()}>{text('logout',lang)}</button></header>
    {msg&&<div className="success">{msg}</div>}
    {context.permissions.manage_profile && <section className="panel"><div className="section-title"><div><h2>{text('profile',lang)}</h2><p className="muted">{text('profileDesc',lang)}</p></div><div className="language-tabs" aria-label={text('editingLanguage',lang)}>{langs.map(x=><button type="button" key={x.id} className={`language-circle ${lang===x.id?'active':''}`} title={x.name} aria-label={`${text('editLanguage',lang)} ${x.name}`} onClick={()=>setLang(x.id)}>{x.label}</button>)}</div></div><div className="grid2"><div className="field"><label>{text('baseName',lang)}</label><input value={profile.name} onChange={e=>setProfile({...profile,name:e.target.value})}/></div><div className="field"><label>{text('slug',lang)}</label><input value={profile.slug} onChange={e=>setProfile({...profile,slug:e.target.value})}/></div><div className="field"><label>{text('nameIn',lang)} {currentLangName}</label><input value={tr.name||''} placeholder={contentFallback(profile.name,translations,'en','name')} onChange={e=>updateProfileTranslation('name',e.target.value)}/></div><div className="field"><label>{text('handleIn',lang)} {currentLangName}</label><input value={tr.handle||''} placeholder={contentFallback(profile.handle,translations,'en','handle')} onChange={e=>updateProfileTranslation('handle',e.target.value)}/></div></div><div className="field"><label>{text('baseBio',lang)}</label><textarea value={profile.bio||''} onChange={e=>setProfile({...profile,bio:e.target.value})}/></div><div className="field"><label>{text('bioIn',lang)} {currentLangName}</label><textarea value={tr.bio||''} placeholder={contentFallback(profile.bio,translations,'en','bio')} onChange={e=>updateProfileTranslation('bio',e.target.value)}/></div><div className="grid2"><div className="field"><label>{text('baseFooter',lang)}</label><input value={profile.footer_text||''} onChange={e=>setProfile({...profile,footer_text:e.target.value})}/></div><div className="field"><label>{text('footerIn',lang)} {currentLangName}</label><input value={tr.footer||''} placeholder={contentFallback(profile.footer_text,translations,'en','footer')} onChange={e=>updateProfileTranslation('footer',e.target.value)}/></div></div><div className="grid2"><div className="field"><label>{text('accent',lang)}</label><input value={profile.accent_color} onChange={e=>setProfile({...profile,accent_color:e.target.value})}/></div>{context.permissions.manage_assets&&<div className="field"><label>{text('churchLogo',lang)}</label><input type="file" accept="image/png,image/jpeg,image/webp" disabled={uploading} onChange={e=>{const f=e.target.files?.[0];if(f)uploadLogo(f)}}/>{profile.logo_url&&<img src={profile.logo_url} alt={text('currentLogo',lang)} style={{width:110,height:110,objectFit:'contain',marginTop:8,borderRadius:'50%',background:'#fff'}}/>}</div>}</div><button className="btn btn-primary" onClick={saveProfile}>{text('saveProfile',lang)}</button></section>}
    {context.permissions.manage_links && <section className="panel"><div className="section-title"><div><h2>{text('links',lang)}</h2><p className="muted">{text('linksDesc',lang)}</p></div><button className="btn btn-primary" onClick={add}>{text('add',lang)}</button></div>{links.map((l,i)=>{const lt=l.translations?.[lang]||{};const english=l.translations?.en||{};return <div className="edit-card" key={l.id||i}><div className="field"><label>{text('baseTitle',lang)}</label><input value={l.title} onChange={e=>{const n=[...links];n[i]={...l,title:e.target.value};setLinks(n)}}/></div><div className="field"><label>{text('titleIn',lang)} {currentLangName}</label><input value={lt.title||''} placeholder={english.title||l.title} onChange={e=>updateLinkTranslation(i,'title',e.target.value)}/></div><div className="field"><label>{text('baseSubtitle',lang)}</label><input value={l.subtitle||''} onChange={e=>{const n=[...links];n[i]={...l,subtitle:e.target.value};setLinks(n)}}/></div><div className="field"><label>{text('subtitleIn',lang)} {currentLangName}</label><input value={lt.subtitle||''} placeholder={english.subtitle||l.subtitle||''} onChange={e=>updateLinkTranslation(i,'subtitle',e.target.value)}/></div><div className="field"><label>{text('url',lang)}</label><input value={l.url} onChange={e=>{const n=[...links];n[i]={...l,url:e.target.value};setLinks(n)}}/></div><div className="field"><label>{text('icon',lang)}</label><select value={l.icon} onChange={e=>{const n=[...links];n[i]={...l,icon:e.target.value};setLinks(n)}}><option value="instagram">Instagram</option><option value="facebook">Facebook</option><option value="youtube">YouTube</option><option value="whatsapp">WhatsApp</option><option value="donation">Donaciones</option><option value="calendar">Eventos</option><option value="website">Web</option><option value="link">Enlace</option></select></div><button className="btn btn-primary" onClick={()=>saveLink(l,i)}>{text('saveLink',lang)}</button></div>})}</section>}
